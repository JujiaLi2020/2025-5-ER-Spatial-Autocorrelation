...
save(out, file = paste0(repos, "/Yao_out.Rdat"))


load(paste0(repos, "/Yao_out.Rdat"))


dir()

# ############################### Loop
# nform <- length(formulas2[[1]])
# 
# # Pre-allocate list to store results
# out <- vector("list", nform)
# 
# # Loop over formulas
# for (i in 1:nform) {
#   out[[i]] <- wrapper(
...
# 
# # Save result
# save(out, file = paste0(repos, "out.Rdat"))
# closeAllConnections()


### Read Rdat
# file_path <- "C:/Users/julia/Box/Personal/Research/2025-5-ER-Spatial-Autocorrelation/Data/out_1205.Rdat"
# load(file_path)
...
final_fit <- getFit(df, df_select_z_int, formulas2[[which(outmatrix == min(outmatrix[, c(2, 4, 6, 8, 10, 12, 14 ,16)]), arr.ind = TRUE)[1]]], spatial = "on", spatiotemporal = "off")
...
# Pre-allocate a list to store the full results
all_model_results <- vector("list", length(formulas2))

for (i in 1:length(formulas2)) {
  cat("Fitting model", i, "of", length(formulas2), "\n")
  
  # Run your getFit function for each formula
  # Note: getFit returns a list: list("init_fit" = init_fit, "fit" = previous_fit)
  model_output <- getFit(
    df = df, 
    df_select_z_int = df_select_z_int, 
    formula = formulas2[[i]], 
  ...
  # Store the fit object and the metrics together
  all_model_results[[i]] <- list(
    index = i,
    formula = formulas2[[i]],
    fit = model_output$fit, # This is the full sdmTMB object
    metrics = outmatrix[i, ] # Your existing CV metrics
  )
}

# Save everything to one file
save(all_model_results, file = "all_models_full.RData")
...
final_coef_table <- map_df(all_model_results, function(x) {
  # Extract tidy coefficients from the stored 'fit' object
  tidy_df <- tidy(x$fit, effects = "fixed", conf.int = TRUE)
  
  # Add the model index and metrics (like AIC or RMSE) to the table
  tidy_df$model_index <- x$index
  # Assuming outmatrix columns have names, we can append them here
  # tidy_df$AIC <- x$metrics["AIC"] 
  
  return(tidy_df)
})


write.csv(final_coef_table, "final_coef_table.csv")

filtered_coefs <- final_coef_table %>%
  filter(str_detect(term, "^ER_annual"))



table(filtered_coefs)

write.csv(filtered_coefs, "ER_coefs.csv")
...
# 1. Calculate AIC for every model in your list
# AIC = 2k - 2 * LogLik
# k is the number of parameters (fixed effects + intercept)
aic_values <- sapply(1:length(formulas2), function(i) {
  # k = count of variables in formula + 1 for intercept
  k <- length(all.vars(formulas2[[i]])) 
  
  # Metric 1 is usually the Log-Likelihood for the whole dataset
  log_lik <- outmatrix[i, 1] 
  
  return(2 * k - 2 * log_lik)
})

# 2. Build the table with Formulas and AIC
model_results_all <- data.frame(
  Model_Index = 1:length(formulas2),
  Formula     = sapply(formulas2, function(x) paste(format(x), collapse = " ")),
  AIC         = aic_values
)

# 3. Sort the table by AIC (Lowest to Highest)
model_results_sorted <- model_results_all[order(model_results_all$AIC), ]

# 4. Add Delta AIC (Difference from the best model)
model_results_sorted$Delta_AIC <- model_results_sorted$AIC - min(model_results_sorted$AIC)

# 5. View the Top 10 Models
head(model_results_sorted, 30)

# 6. Save the sorted ranking to CSV
write.csv(model_results_sorted, "model_ranking_by_aic.csv", row.names = FALSE)



# 1. Extract the 16 metrics for model 65
metrics_65 <- outmatrix[65, ]

# 2. Extract the formula for model 65
formula_65 <- formulas2[[65]]

# Print the formula to confirm it is the one you want
print(formula_65)

fit_65_output <- getFit(
  df = df, 
  df_select_z_int = df_select_z_int, 
...
# 3. Extract the final fit object
model_65_fit <- fit_65_output$fit

model_65_fit


# Manually build the mesh to test
mesh <- make_mesh(
  df,
  xy_cols = c("X","Y"),
  cutoff = diff(range(df$X, na.rm = TRUE)) / 10
...
# Try the fit manually to see the error message
test_fit <- sdmTMB(
  formula = formulas2[[65]],
  data = df,
  mesh = mesh,
  family = poisson(link = "log"), # or nbinom1
  spatial = "on",
  offset = df$log_pop
)
