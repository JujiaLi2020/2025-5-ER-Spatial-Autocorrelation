...
# Build cyclic seasonality + time index (1..T)
er_month <- er_month |>
  ...
  mutate(
    ...
    time_index = match(year_month, sort(unique(year_month)))
  )
```
...
df_month <- er_month |>
  ...
  filter(is.finite(X), is.finite(Y), is.finite(log_pop))
```
...
mesh <- make_mesh(
  df_month,
  xy_cols = c("X","Y"),
  cutoff = diff(range(df_month$X, na.rm = TRUE)) / 10
)
```
