...
#   Deaths_em_rate ~ ER_annual_percapita_z +                 # ER visits
...
#   Deaths_em_rate ~ ER_annual_percapita_z * time_index
# )
# 
...
#### Try Formula for difference imputation
```{r}
## MODEL FORMULAS ( "." = additive, "x" = interaction )

## Full main-effects model (ER . opioids . SES)
form_ER.All <- as.formula(
  Deaths_9 ~ ER_annual_percapita_z +                 # ER visits
    OXYCODONE_annual_percapita_z +                    # opioid types
    HYDROCODONE_annual_percapita_z +
    BUPRENORPHINE_annual_percapita_z +
    METHADONE_annual_percapita_z +
...
## ER only
form_ER <- as.formula(
  Deaths_9 ~ ER_annual_percapita_z
)
...
## ER x Time (interaction)
form_ERxTime <- as.formula(
  Deaths_9 ~ ER_annual_percapita_z * time_index
)
...
### 5.4.1 Explore Selected Four Models in Last Week [New]
```{r}
# Define the model names you want
model_names <- c(
  "Poisson_spOFF_OFF_form_ER",
  "Poisson_spOFF_OFF_form_ERxOpioid.SES",
  "Poisson_spOFF_OFF_form_ERxOpioid.SES",
  "Poisson_spON_OFF_form_ERxOpioid.SES"
)


# Check which exist in the fits list
model_names <- model_names[model_names %in% names(fits)]

# Extract coefficients for each
coef_list <- lapply(model_names, function(m) {
  fit_i <- fits[[m]]
  if (inherits(fit_i, "sdmTMB")) {
    coefs <- broom::tidy(fit_i)
    coefs$model <- m
    coefs
  } else {
    warning(sprintf("Model %s not found or invalid.", m))
    NULL
  }
})

# Combine into one table
coef_table <- dplyr::bind_rows(coef_list)

print(coef_table)


#write.csv(coef_table, "output/coef_table.csv")

```

In 1st and 10 24 27 model, we created coef tables for each, and found that No coef is significant.


```{r}
summary(fit)$coefficients
car::vif(fit)  # for multicollinearity

```
The Variance Inflation Factor quantifies how much a predictor’s variance is inflated due to multicollinearity with other predictors.


 is the R² when variable j is regressed on all the others.
All VIFs are below 5 — acceptable multicollinearity. No need to drop predictors, though disability_rate_z shows some overlap with other socioeconomic indicators.
