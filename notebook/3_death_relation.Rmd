...
### 3.1 [new]EM Imputation of Death
...
### 3.2 [new]Fixed Imputation of Death
...
```{r}
df_imputated%>%
  select(Deaths,Deaths_1,Deaths_5,Deaths_9,Deaths_em)
```
...
## 4. [Updated] Correlation of Predictors
...
## 5. [Updated] Spatiotemporal Model
...
### [new] 5.2 Create Formula

```{r}

form <- as.formula(Deaths_em ~ ER_annual_percapita_z + OXYCODONE_annual_percapita_z +
  HYDROCODONE_annual_percapita_z + BUPRENORPHINE_annual_percapita_z +
  METHADONE_annual_percapita_z + unemployee_rate_z + gini_index_z +
  poverty_rate_z + disability_rate_z + noinsurance_rate_z)
form
...
### [new] 5.3 Model Comparison and Curvature Testing (For EM)
...
Poisson_spON_OFF	Poisson	on	off
Poisson_spON_IID	Poisson	on	iid
Poisson_spON_AR1	Poisson	on	ar1
...
# st_opts      <- c("off","iid","ar1")
# 
# # ---------- function to run grid for a given response ----------
# run_grid_for_response <- function(resp_col, df, mesh, offset_col = "log_pop") {
#   stopifnot(resp_col %in% names(df))
# 
#   # reuse your RHS (change these if you change predictors)
#   rhs <- c("ER_annual_percapita_z","OXYCODONE_annual_percapita_z",
#            "HYDROCODONE_annual_percapita_z","BUPRENORPHINE_annual_percapita_z",
#            "METHADONE_annual_percapita_z","unemployee_rate_z","gini_index_z",
#            "poverty_rate_z","disability_rate_z","noinsurance_rate_z")
# 
#   form <- reformulate(rhs, response = resp_col)
# 
#   fits <- list()
#   rows <- list()
#   i <- 1
...
#         time_arg <- if (st == "off") NULL else "time_index"
#         model_name <- sprintf("%s_sp%s_%s", fam_name, toupper(sp), toupper(st))
#         cat(sprintf("Fitting: %s | response=%s\n", model_name, resp_col))
# 
#         res <- fit_with_warnings(
#           sdmTMB(
#             formula = form,
...
# 
#           pd <- tryCatch(isTRUE(fit_i$sd_report$pdHess), error = function(e) NA)
#           grad_max <- tryCatch(max(abs(fit_i$sd_report$gradient.fixed), na.rm = TRUE),
...
#             spatiotemporal = st,
#             n = nrow(df),
#             n_nonmiss = sum(!is.na(df[[resp_col]])),
#             prop_zero = mean(df[[resp_col]] == 0, na.rm = TRUE),
#             AIC = AIC(fit_i),
#             logLik = as.numeric(logLik(fit_i)),
#             npar = attr(logLik(fit_i), "df"),
#             hessian_pd = pd,
...
# }
# 
# # ---------- run for all responses and combine ----------
# responses <- c("Deaths_em", "Deaths_1", "Deaths_5", "Deaths_9")  # adjust names if needed
# 
# tables <- lapply(responses, function(rsp) run_grid_for_response(rsp, df, mesh))
# aic_table_all <- bind_rows(tables) %>%
#   group_by(response) %>%
#   arrange(AIC, .by_group = TRUE) %>%
#   mutate(DeltaAIC = AIC - min(AIC, na.rm = TRUE)) %>%
#   ungroup()
# 
# print(aic_table_all, n = nrow(aic_table_all))


```

```{r}
# write.csv(aic_table_all, "model_grid_comparison_all_responses.csv", row.names = FALSE)

```


### [new] 5.4 Try the best model
```{r}
# Find the row for the combo
idx <- with(aic_table, which(family == "Poisson" & spatial == "on" & spatiotemporal == "iid"))
...
```
### [new]5.7 Two-part Hurdle [Under tuning]
```{r}
